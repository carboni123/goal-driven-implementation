...
with its mitigation. `None` is valid only after every class was checked. At completion, the trace
is checked back against this list: confirmed, never fired, missed.

Each class below names the failure it exists to prevent. The origins are real runs; the pattern
is what matters.
...
- **Unanchored section** — no input feeds it → scope nobody asked for. Name the input or cut.
- **Unreachable goal** — a diamond with no incoming edge → the plan cannot finish it. Add the
  section or drop the goal.
- **Gateless handoff** — a goal wired straight to PR/handoff, or code reaching an external or
...
- **Convergence bottleneck** — ≥3 hard edges into one node. Sequence it last, state how its
  inputs get verified independently, expect review pressure there, or split it.
- **Long or false blocking chain** — ≥4 sections in one hard chain, or a preference drawn as a
  hard edge. Demote convention-only edges to soft; name which prefix still delivers standalone
  value if the run stalls. _Origin:_ a chain whose "independently valuable prefix" claim was
  wrong because the sections were hard-coupled in the test plane, not the source plane — 58 red
  tests across five modules until the middle section landed.
...
- **Dropped or partially consumed input** — an input node that reaches no section, _or an input
  ...
  build, acceptance run, whole-branch review, and a six-package conformance fan-out.
- **Sibling surface** — the same job shape, guard, or resolver exists in another module. Sweep
  it or exclude it by name. _Origin:_ a submit-retry race fixed in one module and left live in its
  twin, found by a customer.
...
- **Plan as evidence** — every anchor resolves (`validate-report.mjs --kind anchors` over the
  ...
  faithfully shipped and only a code-anchored reviewer caught.
...
- **Ruling-set completeness** — every `⚠` has a ruling, but the _set_ is incomplete: the plan asks
  a later section for data no earlier section puts on the wire, or an error code the plan will
  need is not named. Check the data flow input → wire → consumer for each ruling. _Origin:_ two
  mid-run escalations in one run, both on decisions the table had not anticipated, one of which
  spawned an unplanned section.
- **Unruled semantics (negative space)** — any section whose verb is enforce, gate, block,
  redact, suspend, or pause must pre-rule what stays open: wind-down paths, reads, and every
...
- **Misplaced verification gate** — a cheap check delayed behind unrelated work, or an expensive
  or mutating gate scheduled before a later input invalidates it. Cheap feedback early; expensive
  gates after their final invalidating input.
...
- **Cheapest real-client probe** — a browser page load, one journey leg, one live HTTP burst,
  scheduled _before_ the first image build. _Origin:_ four of ten plans in one week had their real
  defects surfaced only by the most expensive gate in the budget.
...
- **Known blockers** — list every host or environment condition that has blocked this
  repository's gates before, with pre-approved handling. Hitting one is an `⚙` retry, not a
...
- **Evaluator soundness** — a green journey counts only after fault injection turns it red; a
...
- **Claim decay** — a claim true when its section landed and false at branch end; a correction in
  ...
  and the single largest final-review catch class.
- **Reviewer unavailability** — if the reviewer role cannot dispatch, record `review: self` and
  ...
  all self-reviewed after the reviewer route failed silently had the most live-gate defects and blew
  its build budget three times over.
